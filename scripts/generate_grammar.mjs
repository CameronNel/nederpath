...
// Finalize rich metadata across all rules
const finalizedRules = ruleList.map((r, i) => {
  const lvl = r.level || "A1";
  const diff = lvl === "A1" || lvl === "A0" ? "beginner" : lvl === "A2" || lvl === "B1" ? "intermediate" : "advanced";
  const time = lvl === "A1" || lvl === "A0" ? 5 : lvl === "A2" || lvl === "B1" ? 7 : 10;
  
  return {
    ...r,
    difficulty: r.difficulty || diff,
    estimatedTimeMin: r.estimatedTimeMin || time,
    tags: r.tags || [r.sectionTitle.toLowerCase().replace(/[^a-z0-9]+/g, "_"), lvl.toLowerCase()],
    prerequisites: r.prerequisites || (i > 0 ? ["g-" + String(i).padStart(3, "0")] : []),
    relatedRules: r.relatedRules || (i < ruleList.length - 1 ? ["g-" + String(i + 2).padStart(3, "0")] : []),
    structuralBreakdown: r.structuralBreakdown || "[Topic / Fronted Element] + [Finite Verb (V2)] + [Subject] + [Middle Field: TMP] + [Verb Cluster / Separable Prefix]",
    commonMistake: r.commonMistake || `Incorrect word order or inappropriate ending when applying ${r.titleNl.toLowerCase()}.`,
    correction: r.correction || `Strictly adhere to the standard Dutch placement and conjugation rules described above.`
  };
});

console.log(`Generated ${finalizedRules.length} grammar rules across 8 sections.`);
if (finalizedRules.length < 120) {
  throw new Error(`Only ${finalizedRules.length} rules generated, target is >= 120.`);
}
